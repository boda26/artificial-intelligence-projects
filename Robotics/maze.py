...
class Maze:
    def __init__(self, input_map, alien, mst_cache={}, granularity=DEFAULT_GRANULARITY, offsets=[0, 0, 0], filepath=None,
                 use_heuristic=True):
            ...
            for y in range(self.__dimensions[Y]):
                for shape in range(self.__dimensions[SHAPE]):  
                    if self.__map[x][y][shape] == START_CHAR:
                        config = idxToConfig((x, y,shape), self.offsets, granularity,self.__alien)
                        state = (config[0], config[1], alien.get_shapes().index(config[2]))
                        self.__start = state
                    elif self.__map[x][y][shape] == OBJECTIVE_CHAR:
                        config = idxToConfig((x, y,shape), self.offsets, granularity,self.__alien)
                        state = (config[0], config[1], alien.get_shapes().index(config[2]))
                        self.__objective.append(state)
        self.__objective = tuple(self.__objective)
        ...
        self.__start = MazeState(self.__start, self.getObjectives(), 0, self, self.mst_cache, self.use_heuristic)
    ...
    def readFromFile(self, path):
        ...
        # Converts start from tuple to a MazeState
        self.__start = MazeState(self.__start, self.getObjectives(), 0, self, self.mst_cache, self.use_heuristic)
            ...
            if(type(c) == int):
                if(0<= c < len(self.alien.get_shapes())):
                    ...
                    if(self.isValidMove(a,b,c)):
                        neighbors.append((a,b,self.alien.get_shapes().index(c)))
                else:
                    continue
    ...
    def isValidPath(self, path):
        ...
        if not path[-1].state in self.__objective:
