...
class Application:
    ...
    def run(self, filepath, mode, save):
        ...
        if self.human:
            self.agent = agent(self.maze.getStart().state, self.maze)
...
{{
    path length         : {0}
    total execution time: {1:.2f} seconds
}}
            """.format(len(path), time_total))
    ...
    def draw_start(self):
        i, j, k = self.maze.getStart().state
...
if __name__ == "__main__":
    ...
    parser.add_argument('--search', dest = 'search', type = str, default = 'astar',
                        choices = ('astar'), 
                        help = 'search method')
