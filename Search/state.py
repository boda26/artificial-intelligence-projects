from utils import MST, compute_mst_cost, is_english_word, levenshteinDistance
...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        if self.zero_loc[0] != 2:
            below = copy.deepcopy(self.state)
            below[self.zero_loc[0]][self.zero_loc[1]] = below[self.zero_loc[0] + 1][self.zero_loc[1]]
            below[self.zero_loc[0] + 1][self.zero_loc[1]] = 0
            below_state = EightPuzzleState(below, self.goal, self.dist_from_start + 1, self.use_heuristic, (self.zero_loc[0] + 1, self.zero_loc[1]))
            nbr_states.append(below_state)

        if self.zero_loc[1] != 0:
            left = copy.deepcopy(self.state)
            left[self.zero_loc[0]][self.zero_loc[1]] = left[self.zero_loc[0]][self.zero_loc[1] - 1]
            left[self.zero_loc[0]][self.zero_loc[1] - 1] = 0
            left_state = EightPuzzleState(left, self.goal, self.dist_from_start + 1, self.use_heuristic, (self.zero_loc[0], self.zero_loc[1] - 1))
            nbr_states.append(left_state)

        if self.zero_loc[0] != 0:
            above = copy.deepcopy(self.state)
            above[self.zero_loc[0]][self.zero_loc[1]] = above[self.zero_loc[0] - 1][self.zero_loc[1]]
            above[self.zero_loc[0] - 1][self.zero_loc[1]] = 0
            above_state = EightPuzzleState(above, self.goal, self.dist_from_start + 1, self.use_heuristic, (self.zero_loc[0] - 1, self.zero_loc[1]))
            nbr_states.append(above_state)

        if self.zero_loc[1] != 2:
            right = copy.deepcopy(self.state)
            right[self.zero_loc[0]][self.zero_loc[1]] = right[self.zero_loc[0]][self.zero_loc[1] + 1]
            right[self.zero_loc[0]][self.zero_loc[1] + 1] = 0
            right_state = EightPuzzleState(right, self.goal, self.dist_from_start + 1, self.use_heuristic, (self.zero_loc[0], self.zero_loc[1] + 1))
            nbr_states.append(right_state)
    ...
    def compute_heuristic(self):
        ...
        for i in range(3):
            for j in range(3):
                for x in range(3):
                    for y in range(3):
                        if self.state[i][j] != 0 and self.state[i][j] == self.goal[x][y]:
                            total += manhattan((i, j), (x, y))
...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for n in neighboring_grid_locs:
            state = SingleGoalGridState(n, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors)
            nbr_states.append(state)
        return nbr_states
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for loc in neighboring_locs:
            if loc in self.goal:
                tmp = copy.deepcopy(self.goal)
                tmp = list(tmp)
                tmp.remove(loc)
                tmp = tuple(tmp)
            else:
                tmp = self.goal
            state = GridState(loc, tmp, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors, self.mst_cache)
            nbr_states.append(state)
        return nbr_states
    ...
    def compute_heuristic(self):
        ...
        if hash(self.goal) in self.mst_cache:
            mst_val = self.mst_cache[hash(self.goal)]
        else:
            mst_val = compute_mst_cost(self.goal, manhattan)
            self.mst_cache[hash(self.goal)] = mst_val
        min_dist = float('inf')
        for g in self.goal:
            min_dist = min(min_dist, manhattan(self.state, g))
        return min_dist + mst_val
