...
def best_first_search(starting_state):
    ...
    while len(frontier) != 0:
        ...
        if top.is_goal():
            ...
            return path
        for state in top.get_neighbors():
            if hash(state) not in visited_states or state.dist_from_start + state.compute_heuristic() < visited_states[hash(state)][1]:
                visited_states[hash(state)] = (top, state.dist_from_start + state.compute_heuristic())
                #print(state.compute_heuristic())
